...
function hdh_handle_create_offer() {
    ...
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'hdh_create_offer')) { wp_send_json_error(array('message' => 'Güvenlik kontrolü başarısız')); return; }
    $listing_id = isset($_POST['listing_id']) ? absint($_POST['listing_id']) : 0;
    $offerer_id = get_current_user_id();
    if (!$listing_id) { wp_send_json_error(array('message' => 'Geçersiz ilan')); return; }
    $listing = get_post($listing_id);
    if (!$listing || $listing->post_type !== 'hayday_trade') { wp_send_json_error(array('message' => 'İlan bulunamadı')); return; }
    if ($offerer_id == $listing->post_author) { wp_send_json_error(array('message' => 'Kendi ilanınıza teklif yapamazsınız')); return; }
    $listing_status = get_post_meta($listing_id, '_hdh_trade_status', true);
    if ($listing_status !== 'open') { wp_send_json_error(array('message' => 'Bu ilan artık açık değil')); return; }
    ...
    foreach ($existing_offers as $offer) {
        if (isset($offer['offerer_id']) && $offer['offerer_id'] == $offerer_id) {
            wp_send_json_error(array('message' => 'Bu ilana zaten teklif yaptınız')); return;
        }
    }
    ...
    wp_send_json_success(array('message' => 'Teklifiniz gönderildi', 'offer_id' => count($existing_offers) - 1));
}
...
function hdh_handle_offer_response() {
    ...
    if (!$listing_id || $offer_index < 0 || !in_array($action, array('accept', 'reject'))) { wp_send_json_error(array('message' => 'Geçersiz parametreler')); return; }
    $listing = get_post($listing_id);
    if (!$listing || $listing->post_type !== 'hayday_trade') { wp_send_json_error(array('message' => 'İlan bulunamadı')); return; }
    if (get_current_user_id() != $listing->post_author) { wp_send_json_error(array('message' => 'Yetkisiz işlem')); return; }
    $offers = get_post_meta($listing_id, '_hdh_offers', true);
    if (!is_array($offers) || !isset($offers[$offer_index])) { wp_send_json_error(array('message' => 'Teklif bulunamadı')); return; }
    ...
    wp_send_json_success(array('message' => $action === 'accept' ? 'Teklif kabul edildi' : 'Teklif reddedildi'));
}
...
function hdh_handle_complete_exchange() {
    ...
    if ($user_id != $listing_author_id && $user_id != $accepted_offerer_id) { wp_send_json_error(array('message' => 'Yetkisiz işlem')); return; }
    ...
    if ($author_confirmed === '1' && $offerer_confirmed === '1') {
        ...
        wp_send_json_success(array('message' => 'Hediyeleşme tamamlandı!'));
    } else {
        wp_send_json_success(array('message' => 'Onayınız kaydedildi. Diğer tarafın onayını bekliyoruz.'));
    }
}
